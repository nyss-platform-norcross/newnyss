...
export const ExpandButton = ({ onClick, isExpanded }) => {
  ...
  const useStyles = makeStyles({
    triangleBackground: {
      ...
      backgroundImage: useRtlDirection ? 'linear-gradient(-315deg, rgba(255,0,0,0) 50%, #F4F4F4 50%)' : 'linear-gradient(45deg, rgba(255,0,0,0) 50%, #F4F4F4 50%)',
      borderRadius: '10px',
      display: 'flex',
    ...
    button: {
      padding: 0,
      transform: useRtlDirection ? 'rotate(-225deg) translateX(-5px)' : 'rotate(135deg) translateX(-5px)',
      color: "#D52B1E",
      '&:hover': {

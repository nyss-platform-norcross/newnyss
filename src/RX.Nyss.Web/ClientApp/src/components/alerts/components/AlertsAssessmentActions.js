...
export const AlertsAssessmentActions = ({
  ...
  const [openValidationSnackbar, setOpenValidationSnackbar] = React.useState(false);
          ...
          <Grid item xs={6}>
            {
              ...
              <>
                ...
                <Snackbar open={openValidationSnackbar}>
                  <Alert style={{backgroundColor:'#eee'}} onClose={handleCloseValidationSnackbar} severity="info">
                    <AlertTitle>{strings(stringKeys.alerts.assess.alert.eidsrValidation.title)}</AlertTitle>
                    <ul>
                      <li>{strings(stringKeys.alerts.assess.alert.eidsrValidation.connection)}: {validateEidsrResult?.isEidsrApiConnectionRunning ? strings(stringKeys.common.boolean.true) : strings(stringKeys.common.boolean.false)}</li>
                      <li>{strings(stringKeys.alerts.assess.alert.eidsrValidation.integrationValid)}: {validateEidsrResult?.isIntegrationConfigValid ? strings(stringKeys.common.boolean.true) : strings(stringKeys.common.boolean.false)} </li>
                      <li>{strings(stringKeys.alerts.assess.alert.eidsrValidation.validReportsCount)}:  {validateEidsrResult?.areReportsValidCount ? validateEidsrResult?.areReportsValidCount : 0 }</li>
                    </ul>
                    <p>{strings(stringKeys.alerts.assess.alert.eidsrValidation.tip)}</p>
                  </Alert>
                </Snackbar>
              </>
            }

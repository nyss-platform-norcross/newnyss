...
public class ExcelExportService : IExcelExportService
{
        ...
        string title)
    {
        ...
        for (var index = 0; index < exportAlertListResponseDtos.Count; index++)
        {
            ...
            worksheet.Cells[columnIndex, 22].Value = alert.InvestigationComment;
            worksheet.Cells[columnIndex, 23].Value = alert.OutcomeComment;
            worksheet.Cells[columnIndex, 24].Value = alert.SummaryComment;
            worksheet.Cells[columnIndex, 25].Value = alert.PublicHealthActionComment;
        }
    ...
    private static string EscapeCharacters(object data)
    {
        ...
        return value.IndexOfAny(_charsToEscape) > -1
            ? $"{_quote}{value}{_quote}"
            : value;
    }
}

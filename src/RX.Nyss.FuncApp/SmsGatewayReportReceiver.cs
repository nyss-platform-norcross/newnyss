...
public class SmsGatewayReportReceiver
{
        ...
        [Blob("%AuthorizedApiKeysBlobPath%", FileAccess.Read)] string authorizedApiKeys)
    {
        ...
        if (httpRequest.Content != null)
        {
            ...
            {
                _logger.Log(LogLevel.Warning, $"Received an SMS Gateway request with length more than {maxContentLength} bytes. (length: {httpRequest.Content.Headers.ContentLength.ToString() ?? "N/A"})");
